...
def classify_problem(prompt_text: object) -> str:
    ...
    rules: list[tuple[str, Iterable[str]]] = [
        ("Bit Manipulation", [r"\bbit\b", r"\bbinary\b", r"\bxor\b", r"\bshift\b", r"\brotate\b", r"\brotation\b"]),
        ("Cryptography", [r"\bcipher\b", r"\bdecrypt\b", r"\bencrypt\b", r"\bencryption\b", r"\bsecret\b", r"\bsubstitution\b", r"\bcaesar\b"]),
        ("Logic Puzzles", [r"\bpuzzle\b", r"\blogic\b", r"\brule\b", r"\bpattern\b", r"\briddle\b"]),
        ("Sequence Analysis", [r"\bsequence\b", r"\bseries\b", r"\bnext\b", r"\bcontinue\b", r"\bterm\b"]),
        ("Mathematical", [r"\bmath\b", r"\bequation\b", r"\bcalculate\b", r"\bnumber\b", r"\bsum\b", r"\bproduct\b", r"\bprime\b", r"\bdivisor\b", r"\bfraction\b"]),
        ("Geometry", [r"\bshape\b", r"\bangle\b", r"\bdistance\b", r"\bgeometry\b", r"\bcoordinate\b", r"\btriangle\b", r"\bcircle\b", r"\bpolygon\b"]),
        ("Graph/Network", [r"\bgraph\b", r"\bnode\b", r"\bedge\b", r"\bpath\b", r"\bnetwork\b", r"\btree\b"]),
        ("Geography/Navigation", [r"\bmap\b", r"\broute\b", r"\bdirection\b", r"\bnavigation\b", r"\bnorth\b", r"\bsouth\b", r"\beast\b", r"\bwest\b", r"\bcity\b"]),
...
def build_report(examples: pd.DataFrame, by_problem_type: pd.DataFrame, hard_cases: pd.DataFrame) -> str:
    ...
    best_per_type = (
        by_problem_type.sort_values(["problem_type", "exact_match", "support"], ascending=[True, False, False])
    ...
    lines.append("- The strongest signal is that the prompt-template changes barely moved exact-match accuracy; the gap between the best and worst strategies is very small.")
    lines.append("- Few-shot formatting increased similarity scores sharply, but it did not translate into more exact matches, which suggests retrieval confidence and answer correctness are weakly coupled here.")
    lines.append("- Most hard cases are prompts whose surface form resembles many training examples but whose transformation rule is subtle, so nearest-neighbor retrieval returns a plausible but wrong answer.")
    lines.append("- When the prompt is a short or highly structured transformation task, the retriever often latches onto format similarity instead of the underlying operation.")
    lines.append("")
    ...
    if sample_hard.empty:
        ...
        for _, row in sample_hard.iterrows():
            lines.append(f"- [all strategies] {row['problem_type']} | gold={row['gold_answer']} | best_sim={row['best_similarity']:.3f} | failed_strategies={row['failed_strategies']}")
...
def main() -> None:
    ...
    parser.add_argument("--examples", type=str, default="outputs/prompt_experiments_examples.csv")
    parser.add_argument("--out-md", type=str, default="outputs/prompt_error_analysis.md")
    parser.add_argument("--out-summary", type=str, default="outputs/prompt_error_analysis_by_problem_type.csv")
    parser.add_argument("--out-hard-cases", type=str, default="outputs/prompt_error_analysis_hard_cases.csv")
    ...
    per_type = (
        ...
        .sort_values(["problem_type", "exact_match", "support"], ascending=[True, False, False])
    )

    # Prompts with zero correct predictions across all strategies.
    per_prompt = (
        examples.groupby(["id", "problem_type", "prompt", "gold_answer", "prompt_preview"], as_index=False)
        .agg(
            strategies_correct=("exact", "sum"),
            best_similarity=("similarity", "max"),
            mean_similarity=("similarity", "mean"),
            failed_strategies=("strategy", lambda values: ", ".join(sorted(set(values)))),
        )
    ...
    for strategy, frame in examples[~examples["exact"]].groupby("strategy"):
        frame = frame.sort_values(["similarity", "prompt_length"], ascending=[False, False])
        best_wrong_rows.append(frame.iloc[0])
    best_wrong_df = pd.DataFrame(best_wrong_rows) if best_wrong_rows else pd.DataFrame(columns=examples.columns)
    ...
    if not best_wrong_df.empty:
        ...
        print(best_wrong_df[["strategy", "problem_type", "gold_answer", "pred_answer", "similarity", "prompt_preview"]].to_string(index=False))
