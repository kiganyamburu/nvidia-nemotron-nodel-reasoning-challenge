...
def compare_proxy_vs_model():
    ...
    try:
        ...
        print("ERROR: Model generation results not found. Run model_generation_evaluation.py first.")
        return
    
    # Merge on strategy, id to align results
    df_merged = pd.merge(
        df_proxy[['strategy', 'id', 'exact']].rename(columns={'exact': 'proxy_exact'}),
        df_model[['strategy', 'id', 'exact_match']].rename(columns={'exact_match': 'model_exact'}),
        on=['strategy', 'id'],
        how='inner'
    )
    ...
    for strategy in sorted(df_merged['strategy'].unique()):
        ...
        both_correct = ((df_strat['proxy_exact'] == 1) & (df_strat['model_exact'] == 1)).sum()
        both_wrong = ((df_strat['proxy_exact'] == 0) & (df_strat['model_exact'] == 0)).sum()
        proxy_only = ((df_strat['proxy_exact'] == 1) & (df_strat['model_exact'] == 0)).sum()
        model_only = ((df_strat['proxy_exact'] == 0) & (df_strat['model_exact'] == 1)).sum()
    ...
    markdown_report += f"- **Both correct:** {total_both_correct} ({total_both_correct/total:.1%})\n"
    markdown_report += f"- **Both wrong:** {total_both_wrong} ({total_both_wrong/total:.1%})\n"
    ...
    if best_model_strategy != best_proxy_strategy:
        ...
        markdown_report += f"   This suggests the proxy may not fully capture model behavior.\n\n"
    else:
    ...
    model_wins = df_merged[(df_merged['proxy_exact'] == 0) & (df_merged['model_exact'] == 1)]
    proxy_wins = df_merged[(df_merged['proxy_exact'] == 1) & (df_merged['model_exact'] == 0)]
