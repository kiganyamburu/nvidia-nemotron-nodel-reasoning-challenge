...
def main():
    ...
    try:
        ...
        print(
            "  2. Ensure sufficient VRAM (60GB for Nemotron in bfloat16, 15-20GB in 4-bit)"
        )
    ...
    for strategy in strategies:
        ...
        avg_length = (
            sum(generated_lengths) / len(generated_lengths) if generated_lengths else 0
        )
        print(
            f"  Exact-match accuracy: {accuracy:.4f} ({exact_matches}/{total_prompts})"
        )
    ...
    print("=" * 80)
    print(
        "1. Run scripts/compare_proxy_vs_model.py to compare results with TF-IDF proxy"
    )
